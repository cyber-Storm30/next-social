...
const CommentCard: React.FC<CommentCardInterface> = (props) => {
  ...
  const [tempReplies, setTempReplies] = useState<any[]>([]);
  const submitComment = async (comment: string) => {
    ...
    setTempReplies([res.data, ...tempReplies]);
  };
  ...
  return (
    <div>
      ...
      <div className={styles.replySection}>
        {tempReplies.map((reply: any, idx: number) => (
          <ReplyComment data={reply} key={idx} />
        ))}
      ...
      {isReplyMode && (
        <CommentInput submitComment={submitComment} isReply={true} />
      )}
    </div>
  );
};
